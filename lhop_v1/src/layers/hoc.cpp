...
histogram_distance_function* histogram_distance_function::new_instance(const string& type_str, const config_dictionary& cfg) {
	histogram_distance_function* distance_func = nullptr;
	
	if (type_str.compare("gaussian") == 0 ) {
		distance_func = new gaussian_distance_f(cfg.get_value_int("sigma", 1.0, true));
		...
		distance_func = new rounded_overlap_distance_f(cfg.get_value_double("sigma", 1, true), cfg.get_value_double("region_extend", 1.2, true));
	} else if (type_str.compare("multikernel_gaussian") == 0 ) {
		distance_func = new multikernel_gaussian_distance_f(cfg.get_value_double("sigma", 0.7, true), cfg.get_value_double("threshold", 0.1, true));		
...
histograming_impl* histograming_impl::new_instance(part_lib* library, const int current_layer, const string& type_str, const config_dictionary& cfg) {
	histograming_impl* hist_impl = nullptr;

	bool include_similar_parts = cfg.get_value_bool("parts_similarity.enable", false);
	float similarity_sigma = cfg.get_value_bool("parts_similarity.sigma", 0.25);

	bool use_part_merging = cfg.get_value_bool("use_part_merging", true);
	...
	if (type_str.compare("plain_hoc") == 0) {
		...
		hist_impl = new upper_layer_filter_histograming(cfg.get_value_int("top_part_layer", current_layer), current_layer, type_count, merged_parts_indexes);
		...
		cfg.get_value(feature_hist_str, "feature_histograming", false);

		config_dictionary feature_hist_cfg;
		feature_hist_cfg.from_namespace_priority(cfg, 1, "feature_histograming");
		...
		cfg.get_value(feature_bin_str, "feature_binning", false);

		config_dictionary feature_bin_cfg;
		feature_bin_cfg.from_namespace_priority(cfg, 1, "feature_binning");

		binning_impl* local_feature_bin_impl = binning_impl::new_instance(library, current_layer, feature_bin_str, feature_bin_cfg);
		
		int stride = cfg.get_value_int("stride", 2);
		int window_size_x = cfg.get_value_int("window_size_x", 16);
		int window_size_y = cfg.get_value_int("window_size_y", 16);

		string cluster_filename;
		cfg.get_value(cluster_filename, "cluster_centers", true);
...
binning_impl* binning_impl::new_instance(part_lib* library, const int current_layer, const string& type_str, const config_dictionary& cfg) {
	binning_impl* bin_impl = nullptr;

	if (type_str.compare("matrix_binning") == 0) {
		
		bool legacy_compatible = cfg.get_value_bool("legacy_compatible", true);	
		bool interpolate_to_neighboors = cfg.get_value_bool("interpolate_to_neighboors", false);	
		
		histogram_distance_function* distance_func = nullptr;
		string dist_func_str;

		if (cfg.get_value(dist_func_str, "bin_distance_function", false)) {
			config_dictionary cfg_dist_func;
			// copy values based on namespace hierarhy
			cfg_dist_func.from_namespace_priority(cfg,1,"bin_distance_function");

			distance_func = histogram_distance_function::new_instance(dist_func_str, cfg_dist_func);
		}

		string matrix_filename;
		cfg.get_value(matrix_filename, "matrix", true);
		...
		bin_impl = new spm_binning(cfg.get_value_int("pyramid_layers", 3));
	} else if (type_str.compare("grid_binning") == 0) {
		bin_impl = new grid_binning(cfg.get_value_int("width", 0),cfg.get_value_int("height", 0));
	} else {
...
void hoc_histogram_generator::init_cfg(const config_dictionary& cfg) {
	
	// get list of layers on which we will process 
    cfg.get_value(layers, "layer", true);    

	// sliding windows cfg
    sw_xstep = cfg.get_value_int("sliding_window_xstep", 0);
    sw_ystep = cfg.get_value_int("sliding_window_ystep", 0);
	sw_width = cfg.get_value_int("sliding_window_width", 0);
    sw_height = cfg.get_value_int("sliding_window_height", 0);

	padding_x = cfg.get_value_int("padding_x", 0);
	padding_y = cfg.get_value_int("padding_y", 0);

	// we should always be using img coordinates 
	bool use_img_coordinates = cfg.get_value_bool("use_img_coordinates", true);	// make sure it is not FALSE as this is not supported any more !!!!
	...
	if (cfg.get_value(histograming_str, "histogram_implementation", false))
		cfg_hist.from_namespace_priority(cfg,1,(const char*)histograming_str.c_str());
	else 
		cfg_hist = cfg;

	string binning_str("matrix_binning");
	config_dictionary cfg_bin;
	if (cfg.get_value(binning_str, "binning_implementation", false))
		cfg_bin.from_namespace_priority(cfg,1,(const char*)binning_str.c_str());
	else
		cfg_bin = cfg;
