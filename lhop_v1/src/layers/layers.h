...
// This class is exported from the layers.dll
//class LAYERS_API Clayers {
//public:
//	Clayers(void);
//	// TODO: add your methods here.
//};
//
//extern LAYERS_API int nlayers;
//
//LAYERS_API int fnlayers(void);
...
namespace laydisplay {
	/// Removes texture
	/// \param result inferred image
	/// \param layer layer on which to perform the removal 
	/// \param radius radius for texture removal
	/// \param countthresh maximal no. of different part types allowed
	int remove_texture(layer1_result* result, int layer, int radius, int countthresh);
...
public:
	...
	void initialize(const config_dictionary& cfg_learn);     
	void cleanup();
	...
	config_dictionary& get_cfg() { return cfg; }
