...
/*    template<class S> matrix(const matrix<S>& m)
        : std::vector<T>(m.width*m.height), width(m.width), height(m.height) 
    { 
        iterator desti;
        std::vector<S>::iterator srci;
        for (desti = begin(), srci = begin(); desti != end(); ++desti, ++srci) *desti = *srci;
    }*/
...
/*
    template<class S> matrix& operator=(const matrix<S>& src)
    {
        this->resize(src.width, src.height);
        int i;
        for_each(*this, i) { (*this)[i] = src[i]; }
        return *this;
    }
*/
