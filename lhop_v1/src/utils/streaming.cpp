...
void class_id_store::register_class(streamable* c)
{
    int new_id = (int)id_map.size();
    
    id_map.push_back(c);
    if (c != nullptr)
        name_map.insert(pair<string, int>(typeid(*c).name(), new_id));
}
...
////////////////
//
//ofzstreamer::ofzstreamer() :
//    ostreamer(&fbuf)
//{
//}
//
//void ofzstreamer::open(const char* fname)
//{
//    fbuf.open(fname, ios::out | ios::binary | ios::trunc);
//    if (fail()) throw streaming_exception(__FILE__, __LINE__,streaming_exception::CANNOT_OPEN_FILE, fname);
//}
//
//void ofzstreamer::close()
//{
//    fbuf.close();
//}
...
/*void ostreamer::write(const std::vector<streamable*>& v)
{
    unsigned size = (unsigned)v.size();

    write(size);
    for (unsigned i = 0; i < size; ++i) write(v[i]);
}*/
...
////////////////
//
//ifzstreamer::ifzstreamer() :
//    istreamer(&fbuf)
//{ }    
//
//void ifzstreamer::open(const char* file)
//{
//    fbuf.open(file, ios::in | ios::binary);
//    if (fail()) throw streaming_exception(__FILE__, __LINE__,streaming_exception::CANNOT_OPEN_FILE, file);
//}
//
//void ifzstreamer::close()
//{
//    fbuf.close();
//}
...
/*        if (id < (int)ptr_store.size()) {
        p = ptr_store[id];
    } else if (id < id_read) {
        id_map.insert(pair<int, streamable**>(id, &p));
    } else {
        ++id_read;
        ptr_store.push_back(nullptr);
        streamable* c = streamable::read_from_stream(*this);
        p = c;
        ptr_store[id] = c;

        multimap<int, streamable**>::iterator iter = id_map.find(id);
        for (iter = id_map.find(id); iter != id_map.end() && iter->first == id; ++iter) {
            *(iter->second) = c;
        }
    }*/
}
...
/* void istreamer::read(vector<streamable*>& v)
{
    unsigned size;
    streamable* s;
...
//class_id_store streamable::class_id_storage;
