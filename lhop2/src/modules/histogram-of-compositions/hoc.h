...
/// @addtogroup hoc Module - Histogram of Compositions
/// @{
...
struct histogram_distance_function {
	...
	static histogram_distance_function* new_instance(const string& type_str, const ConfigDictionary& cfg);
};
...
struct all_cetners_distance_f : public histogram_distance_function {
	...
	virtual double operator()(M_bin& b, ipoint2 nd_loc, int current_bin) { 
		...
		if (loc_distance->second.size() <= 0)  {
			...
			size_t closes_count = min<size_t>(4, distances.size());
...
struct rounded_overlap_distance_f : public histogram_distance_function {
	...
	virtual double operator()(M_bin& b, ipoint2 nd_loc, int current_bin) { 
		...
		float bin_max_dist2 = b.bin_max_distance2(current_bin) * (float)region_extention;
...
public:
	...
	size_t width(int bin) { return gaussian_kernels[bin].width; }
	size_t height(int bin) { return gaussian_kernels[bin].height; }
    ...
    void calculate(M_bin& b, double sigma, double threshold) {      
		...
		float min_kernel_dx = FLT_MAX;
		float min_kernel_dy = FLT_MAX;
		...
		for (int i = 0; i < b.bin_count(); ++i) {
			...
			kernel_dx[i] = 1 / (float)(count - 1) * dx * (float)sigma* (float)sigma ;		
			kernel_dy[i] = 1 / (float)(count - 1) * dy * (float)sigma* (float)sigma;
		...
		for (int i = 0; i < b.bin_count(); ++i) {
			...
			for (int x = 0; x < b.width(); x++ ) {
				for ( int y = 0; y < b.height(); y++) {
					int xx = b.bin_center(i).x - (x - c.y);
					int yy = b.bin_center(i).y - (y - c.y);
...
public:	
	...
	static histograming_impl* new_instance(part_lib* library, const int current_layer, const string& type_str, const ConfigDictionary& cfg);
};
...
public:	
	...
	static binning_impl* new_instance(part_lib* library, const int current_layer, const string& type_str, const ConfigDictionary& cfg);
...
public:
	...
	public:
		...
		inline bool move_next() {
			...
			//        node* n = node_at(x, y, layer);
			//        while (n != nullptr)
			//			  recurse05(start_node_list, EdgeConnection::TO_PREV_LAYER, layer_check_2(layer), to_layer_result);
			...
			if (to_layer_result.size() > 0 && ++to_layer_iter != to_layer_result.end()) {
				...
				while (to_layer_result.size() <= 0 && has_next) {
					...
					if (top_part_n != nullptr) {
						...
						res->recurse05(start_node_list, EdgeConnection::TO_PREV_LAYER, upper_layer_filter_histograming::layer_check(self->layer), to_layer_result);
		...
		inline int type() { return self->merged_parts_mapping[nd->m]; }
		inline float value() { return (float)nd->vval(); }
