...
#include "utils/serialization/zlib_compress.h"
...
public:
    enum TYPE { UNKNOWN_ERROR = 0, CANNOT_OPEN_FILE, CLASS_NOT_REGISTERED, POINTER_ID_NOT_FOUND, NONSTREAMABLE_CLASS, REGISTRATION_CLASS_ID_CLASH };
...
public:    
	...
	template <class T>
    void register_class(int id = -1);
};


template <class T>
void class_id_store::register_class(int id )
{	
	// use user supplied id or generate new one
    int new_id = id >= 0 ? id : (int)id_map.size();
    
	if (id_map.size() <= new_id)
		id_map.resize(new_id+1, nullptr);

	streamable* &intsance_obj = id_map[new_id];

	// check for ID clash (there should not be any pointer at the same location)
	if (intsance_obj != nullptr) {
		stringstream ss;
		ss << "Trying to register '" << typeid(T).name() << "' with existing id '" << id << "' ('" << typeid(*intsance_obj).name() << "' already registred with the same ID)";
		
		cout << streaming_exception(__FILE__, __LINE__, streaming_exception::REGISTRATION_CLASS_ID_CLASH, ss.str()).what() << endl;
		exit(0);
	}

	intsance_obj = new T();
    
    name_map.insert(pair<string, int>(typeid(T).name(), new_id));
}
...
template<class T, unsigned ID> typename streamable::registrar<T,ID>::proxy streamable::registrar<T,ID>::p;

template<typename T, unsigned ID> 
void streamable::register_class(){
	streamable::registrar<T,ID>::p;
}

template<typename T, unsigned ID>
streamable::registrar<T,ID>::proxy::proxy() { 
	//cout << "constructor of proxy called with type name: " << typeid(T).name() << " and id " << ID << endl;
	streamable::get_class_id_store().register_class<T>(ID);
}
...
public:
	...
	void write(const int64_t& i) { write((char*)&i, sizeof(int64_t)); }
	void write(const uint64_t& i) { write((char*)&i, sizeof(uint64_t)); }
...
public:
	...
	void read(int64_t& i) { read((char*)&i, sizeof(int64_t)); }
	void read(uint64_t& i) { read((char*)&i, sizeof(uint64_t)); }
